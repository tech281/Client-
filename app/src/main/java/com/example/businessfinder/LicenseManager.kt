...
object LicenseManager {
    ...
    private const val SECRET_KEY = "MySecretKey12345" // 16 chars for AES-128
    ...
    fun generateLicense(deviceId: String, expiryDays: Int, maxDevices: Int): String {
        val expiry = System.currentTimeMillis() + expiryDays * 24L * 60 * 60 * 1000
        val data = "$deviceId|$expiry|$maxDevices|${System.currentTimeMillis()}"
        return encrypt(data)
    }
    ...
    fun validateLicense(context: Context, licenseCode: String): Boolean {
        return try {
            ...
            val maxDevices = parts[2].toInt()
            // Check if device ID matches (optional, can be loosened for reseller)
            // For simplicity, we don't enforce device binding here; just expiry.
            Date().time < expiry
        } catch (e: Exception) {
            false
        }
    ...
    fun getSavedLicense(context: Context): String? {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getString(KEY_LICENSE, null)
    }

    fun isLicenseValid(context: Context): Boolean {
        val code = getSavedLicense(context) ?: return false
